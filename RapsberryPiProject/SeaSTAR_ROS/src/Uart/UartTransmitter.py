...
class TXManager:
    ...
    def send_hello(self):
        ...
        if self.driver.comm_ok:
            return

        if now < self.next_hello_time:
            return
    ...
    def send_latest_thruster(self):
        # Only thrusters are blocked when STM32 communication is down
        if not self.driver.comm_ok:
            return
    ...
    def cleanup_stale_pings(self):
        ...
        with self.ping_lock:
            ...
            if len(stale) > 0:
                ...
                self.driver.publish_event("UART_COMMUNICATION_LOST")
