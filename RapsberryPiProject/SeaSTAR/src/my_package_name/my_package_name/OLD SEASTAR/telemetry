...
class TelemetryRecorder(Node):
    def __init__(self):
        ...
        self.writer = None
        self.filename = None
        self.latest_filename = "/home/starfy/Capstone/telemetry_latest.csv"
...
#ROS2 subscriptions 
        ...
        self.create_subscription( #subscribe to /uart/telemetry_enviornmental.
            Float32MultiArray,
            '/uart/telemetry_enviornmental', #[temp, depth, pressure, time].
...
#-----------------------------------------------------------------------------------
    def start_recording(self): #start csv.
        ...
        filename = datetime.now().strftime("telemetry_%Y-%m-%d_%H-%M-%S.csv") #it should save the file with the current time. But this might be a problem since we are IN THE MIDDLE OF THE OCEAN!!!
        self.filename = "/home/starfy/Capstone/" + filename #file path of where the csv file will be saved.
