...
#define NUM_CHANNELS 8 // number of pwm possible channels
 ...
 #define SERVO_CLOSE 2450
 #define SERVO_OPEN  1600
...
char PWM_SetThrusterPeriods(uint16_t p1, uint16_t p2, uint16_t p3, uint16_t p4, uint16_t p5) { 
    if ((p1<1000 || p1>2000) || (p2<1000 || p2>2000) || (p3<1000 || p3>2000) || (p4<1000 || p4>2000) || (p5<1000 || p5>2000)) { 
        //printf("period out of range, %ld, %ld, %ld, %ld, %ld\r\n", p1, p2 ,p3, p4, p5);
        return ERROR;
    }
...
uint16_t PWM_GetPeriod(PWM PWM_x){
    uint16_t period = 0;
    switch(PWM_x.mask) { 
        case 0x1: // PWM_1
            period = TIM3->CCR1/2;
...
char PWM_SampleOpenPosition(){
    TIM4->CCR4 = (uint32_t)(2*SERVO_OPEN);
    return SUCCESS;
}

char PWM_SampleClosedPosition(){
    TIM4->CCR4 = (uint32_t)(2*SERVO_CLOSE);
    return SUCCESS;
}
