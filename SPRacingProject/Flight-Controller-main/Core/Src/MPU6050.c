...
void MPU6050_Read_All(MPU6050_t *data){
    ...
    HAL_I2C_Mem_Read(mpu_handle, MPU6050_ADDR, MPU6050_REG_ACCEL_XOUT_H, 1, raw_data, 14, i2c_timeout);



    data->Ax = (int16_t)(raw_data[0] << 8 | raw_data[1])/(16384.0*0.5) + 0.13;
    data->Ay = (int16_t)(raw_data[2] << 8 | raw_data[3])/(16384.0*0.5) - 0.01;
    data->Az = (int16_t)(raw_data[4] << 8 | raw_data[5])/(16384.0*0.5) + 0.85;
